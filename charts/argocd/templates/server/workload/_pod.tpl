...
serviceAccountName: sa-{{ include "common.fullname" . }}-server
imagePullSecrets:
...
containers:
  ...
  - name: server
    ...
    {{- with $ctrs.server.command }}
    command: 
      {{- . | toYaml | nindent 6 }}
    {{- end }}
    {{- with $ctrs.server.args }}
    args: 
      {{- . | toYaml | nindent 6 }}
    {{- end }}
    {{- with $ctrs.server.securityContext }}
    securityContext: 
      {{- . | toYaml | nindent 6 }}
    {{- end }}
    {{- with $ctrs.server.workingDir }}
    workingDir: {{ . | quote }}
    {{- end }}
    {{- with $ctrs.server.resources }}
    resources:
      {{- . | toYaml | nindent 6 }}
    {{- end }}
    env:
      ...
      - name: ARGOCD_SYNC_WITH_REPLACE_ALLOWED
        valueFrom:
          configMapKeyRef:
            ...
            optional: true
    envFrom:
      {{- with ( concat $ctrs.server.extraEnvFrom 
                        $pod.extraEnvFrom) }}
      {{- . | toYaml | nindent 6 }}
      {{- end }}
      - secretRef:
    ...
    ports:
      ...
      {{- end }}
    volumeMounts:
      {{- with ( concat $ctrs.server.extraVolumeMounts 
      ...
      - mountPath: /home/argocd/params
        name: argocd-cmd-params-cm
    {{- with $ctrs.server.startupProbe }}
    startupProbe:
      {{- . | toYaml | nindent 6 }}
    {{- end }}
    {{- with $ctrs.server.livenessProbe }}
    livenessProbe:
      {{- . | toYaml | nindent 6 }}
    {{- end }}
    {{- with $ctrs.server.readinessProbe }}
    readinessProbe:
      {{- . | toYaml | nindent 6 }}
    {{- end }}
{{- end }}